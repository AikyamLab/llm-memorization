...
class GPTNeoSelfAttention(nn.Module):
    ...
    def _attn(self, query, key, value, attention_mask=None, head_mask=None):
        ...
        if self.disable_attn:
            ...
            if q==1:
                attn_weights = torch.zeros((q, k), device=attn_weights.device, dtype=attn_weights.dtype)
                attn_weights[0, -1] = 1
                attn_weights = attn_weights[None, None, :, :]
                attn_weights = attn_weights.repeat(b, h, 1, 1)
            else:
                #Case for k queries and k keys
                attn_weights = torch.eye(q, device=attn_weights.device, dtype=attn_weights.dtype)[None, None, :, :]
                attn_weights = attn_weights.repeat(b, h, 1, 1)
...
class GPTNeoBlock(nn.Module):
    ...
    def forward(
        ...
        if use_cache:
            outputs = (hidden_states,) + (attn_output,) + outputs
        else:
            outputs = (hidden_states,) + (attn_output,) + outputs[1:]
...
class GPTNeoForCausalLM(GPTNeoPreTrainedModel):
    ...
    def apply_short_circuiting(self, layers_to_short_circuit):
        for layer in layers_to_short_circuit:
            self.transformer.h[layer].attn.attention.disable_attn = True

    def reset_short_circuiting(self):
        for layer in self.transformer.h:
            layer.attn.attention.disable_attn = False
